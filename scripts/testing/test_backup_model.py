...
async def test_text_generation(model: str, api_key: str | None) -> bool:
    ...
    print(f"\n--- Test 1: Text generation ({sanitize_log_input(model)}) ---")
...
async def test_streaming(model: str, api_key: str | None) -> bool:
    ...
    print(f"\n--- Test 2: Streaming ({sanitize_log_input(model)}) ---")
    chunks: list[str] = []
    start = time.monotonic()
    try:
        result = await call_backup_model(
...
async def test_tool_calling(model: str, api_key: str | None) -> bool:
    ...
    print(f"\n--- Test 3: Tool calling ({sanitize_log_input(model)}) ---")
    tools = [
        {
            "name": "get_weather",
...
async def test_simulate_fallback(model: str, api_key: str | None) -> bool:
    ...
    print(f"\n--- Test 4: Full agent fallback simulation ({sanitize_log_input(model)}) ---")
    start = time.monotonic()
    try:
...
async def main():
    ...
    print(f"Backup model: {sanitize_log_input(model)}")
