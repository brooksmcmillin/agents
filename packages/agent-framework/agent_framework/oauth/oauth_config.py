...
class OAuthConfig:
    ...
    def supports_device_flow(self) -> bool:
        """Check if OAuth 2.0 Device Authorization Grant (RFC 8628) is supported."""
        # Check for device authorization endpoint
        if self.device_authorization_endpoint:
            return True

        # Check if device_code grant type is in supported grant types
        if self.grant_types_supported:
            device_grant_types = [
                "urn:ietf:params:oauth:grant-type:device_code",
                "device_code",
            ]
            return any(gt in self.grant_types_supported for gt in device_grant_types)
...
async def discover_oauth_config(base_url: str) -> OAuthConfig:
    ...
    async with httpx.AsyncClient() as client:
        ...
        config = OAuthConfig(
            ...
            device_authorization_endpoint=auth_metadata.get("device_authorization_endpoint"),
        ...
        logger.debug(f"Supports device flow: {config.supports_device_flow()}")
